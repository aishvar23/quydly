...
//
// Security:
//   • Every action requires a valid HMAC-SHA256 token over "<postId>:<action>"
//     keyed by SOCIAL_REVIEW_SECRET (the same secret the email signs with).
...
// MUST match review-notify.js signAction.
function expectedToken(postId, action, secret) {
  return createHmac("sha256", secret).update(`${postId}:${action}`).digest("hex");
}

function tokenValid(postId, action, token, secret) {
  if (!secret || !token) return false;
  const expected = Buffer.from(expectedToken(postId, action, secret));
...
function confirmPage(res, { post, action, token, headline }) {
  ...
  // The form POSTs back to this same URL (query carries post/action/token); the
  // mutation only happens here, on submit — not on the GET that rendered this.
  const formAction = `/api/social-review?post=${encodeURIComponent(post)}&action=${encodeURIComponent(action)}&token=${encodeURIComponent(token)}`;
  return sendHtml(res, 200, shell(`${verb}?`, `
    <h2 style="margin:0 0 8px">${verb}?</h2>
    ${headline ? `<p style="color:#e5e7eb;margin:0 0 4px"><b>${headline}</b></p>` : ""}
...
export async function core(req, res, supabase) {
  ...
  if (!tokenValid(post, action, token, secret)) {
    return message(res, 403, "Link could not be verified", "This review link is invalid or has been tampered with.");
  }
  ...
  if (req.method === "GET") {
    ...
    return confirmPage(res, { post, action, token, headline });
  }
  ...
  if (req.method === "POST") {
    ...
    const { data, error } = await supabase
      ...
      .eq("id", post)
      .eq("status", "PENDING_REVIEW") // idempotent: only a still-pending post flips
      .select("id");
    if (error) {
      return message(res, 500, "Something went wrong", "Could not update the post. Please try again.");
    }
    if (!data || !data.length) {
      const { data: cur } = await supabase.from("social_posts").select("status").eq("id", post).maybeSingle();
      return message(res, 200, "Already handled",
        `No change made — this post is already <b>${cur?.status || "not in review"}</b>.`);
    }
