...
async function run(ctx) {
  ...
  console.log('[04-voice-generation] Generating narration audio...');
  ...
  if (isStub) {
    console.warn('[04-voice-generation] Running with stub audio — real output requires ELEVENLABS_API_KEY');
  }

  console.log(`[04-voice-generation] ✓ Audio ${isStub ? '(stub)' : ''} → ${path.basename(outPath)} (${totalDuration.toFixed(1)}s)`);
