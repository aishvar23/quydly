...
async function run(ctx) {
  ...
  console.log('[07-subtitle-generation] Building subtitle cues...');

  const cues = buildCues(script.full_script, alignment, outroStart);

  // Write SRT for reference / fallback burn-in
  const srtPath = path.join(outputDir, `story-${storyId}-subtitles.srt`);
  fs.writeFileSync(srtPath, formatSRT(cues), 'utf8');

  // Write JSON cues for Remotion SubtitleLayer
  const cuesPath = path.join(outputDir, `story-${storyId}-subtitle-cues.json`);
  fs.writeFileSync(cuesPath, JSON.stringify(cues, null, 2), 'utf8');

  console.log(`[07-subtitle-generation] ✓ ${cues.length} cues (suppressed after ${outroStart.toFixed(1)}s)`);
  return { subtitleCues: cues, srtPath, subtitleCuesPath: cuesPath };
}
