...
const stages = [
  ...
  { state: JOB_STATES.SCRIPT_READY,     module: require('./stages/03-script-generation') },
  { state: JOB_STATES.VOICE_READY,      module: require('./stages/04-voice-generation') },
  { state: JOB_STATES.BEATS_ALIGNED,    module: require('./stages/05-beat-alignment') },
  { state: JOB_STATES.ASSETS_READY,     module: require('./stages/06-asset-resolution') },
  { state: JOB_STATES.SUBTITLES_READY,  module: require('./stages/07-subtitle-generation') },
  { state: JOB_STATES.RENDER_READY,     module: require('./stages/08-remotion-render') },
  { state: JOB_STATES.EXPORTED,         module: require('./stages/09-ffmpeg-export') },
  { state: JOB_STATES.METADATA_READY,   module: require('./stages/10-metadata-generation') },
];
...
async function runPipeline({ storyId, audienceGeo, mode, skipAudit }) {
  ...
  console.log(`\n=== Quydly Animated Video Pipeline ===`);
  ...
  for (const stage of stages) {
    try {
      ...
      writeFailureLog(outputDir, stage.state, err);
      process.exit(1);
    }
  }
...
function writeFailureLog(outputDir, state, err) {
  const log = { failed_at: state, error: err.message, stack: err.stack, timestamp: new Date().toISOString() };
  fs.writeFileSync(path.join(outputDir, 'pipeline-failure.json'), JSON.stringify(log, null, 2));
}

function writeRejectionLog(outputDir, reason) {
  fs.writeFileSync(path.join(outputDir, 'video-rejected.json'), JSON.stringify({ reason, timestamp: new Date().toISOString() }, null, 2));
}
