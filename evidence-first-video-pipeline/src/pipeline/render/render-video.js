...
function prepareRenderProps(storyPackage, outputDir) {
  const jobKey = safeName(`evidence-story-${storyPackage.story.id}-${storyPackage.version || 'v1'}`);
  ...
  const musicSrc = copyMusic(storyPackage.music?.audioPath, publicDir, jobKey);
  const modules = storyPackage.modules.map((module) => ({
    ...
    assetClass: module.assetClass,
    data: module.data || {},
    asset: copyAsset(module.asset, module.moduleId, publicDir, jobKey),
  }));
  ...
  const props = {
    ...
    audioSrc,
    musicSrc,
    musicMix: storyPackage.music || null,
    brandName: 'QUYDLY',
    jobKey,
    modules,
...
function renderVideo(storyPackage, outputDir, { skipRender = false } = {}) {
  ...
  if (!fs.existsSync(remotionBin)) {
    throw new Error('Remotion is not installed. Run npm install in evidence-first-video-pipeline.');
  }
...
function copyMusic(audioPath, publicDir, jobKey) {
  if (!audioPath || !fs.existsSync(audioPath) || fs.statSync(audioPath).size === 0) return null;
  const dest = path.join(publicDir, 'background-music.wav');
  fs.copyFileSync(audioPath, dest);
  return path.posix.join('jobs', jobKey, 'background-music.wav');
}

function copyAsset(asset, moduleId, publicDir, jobKey) {
  if (asset?.people?.length) {
    const people = asset.people.map((personAsset, index) => (
      copyIndividualAsset(personAsset, `module-${moduleId}-person-${index + 1}`, publicDir, jobKey)
    ));
    const primary = people.find((personAsset) => personAsset.src) || null;
    return {
      ...asset,
  ...
  return copyIndividualAsset(asset, `module-${moduleId}`, publicDir, jobKey);
}

function copyIndividualAsset(asset, basename, publicDir, jobKey) {
