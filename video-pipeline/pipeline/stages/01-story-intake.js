...
// Stage 01: Fetch and validate a story from Supabase.
// POC mode: allows is_verified=false with a warning.
// Production mode: blocks unverified stories.
...
async function run(ctx) {
  ...
  if (!Array.isArray(story.key_points) || story.key_points.length < 3)
                                                              errors.push('fewer than 3 key_points');
  if (story.confidence_score < 6)                             errors.push(`confidence_score ${story.confidence_score} < 6`);
  // Only gate on quality scores when they are present — null means audit not yet run
  ...
  if (!story.is_verified) {
    ...
    console.warn('[story-intake] WARNING: story is_verified=false — running in POC/internal mode only');
  }
