...
const SYSTEM_PROMPT = `You write short-form news explainer video scripts.
...
SCENE SEQUENCE (follow this order — pick safe_visual_concept from the preferred list):
${sceneGuidance}
...
SCRIPT REQUIREMENTS:
- total 80–110 words
- tone: factual, concise, natural for spoken narration
- no clickbait, no speculation
- structure: hook → main event → key detail → charges or consequence → why it matters → close
...
For each scene output ONLY these fields:
  ...
  overlay_text        3–5 WORDS MAXIMUM — punchy editorial strap. Scene 1 MUST be ultra-short and curiosity-driving: like "$400K from secret intel?" or "Classified intel. Cash bets." or "Secret intel. $400K." — other scenes: like "Wire fraud filed" or "Bets placed on ops" — NOT a full sentence, NOT a subtitle repeat
  scene_purpose       MUST be exactly one of: ${VALID_PURPOSES.join(' | ')}
  safe_visual_concept MUST be exactly one of: ${VALID_CONCEPTS.join(' | ')} — pick from the preferred list for each scene position
...
CRITICAL RULES:
- overlay_text is a 4–6 word editorial strap, never a full sentence
- Do NOT generate search queries — only safe_visual_concept from the enum
- Do NOT imply direct footage of named real events or individuals
- Scene 6 (outro) must always use safe_visual_concept: brand_outro
- Scene 6 narration_segment must be empty string ""
  ...
  "scenes": [
    {
      "scene_id": 1,
      "type": "hook",
      "overlay_text": "Classified intel. Cash bets.",
...
async function run(ctx) {
  ...
  console.log(`[02-script-generation] Story type: ${storyType}`);
  console.log('[02-script-generation] Calling Claude for script + scene plan...');
  ...
  for (const scene of parsed.scenes) {
    ...
    // Enforce overlay_text length — truncate silently if Claude went over
  ...
  // Placeholder durations (beat alignment refines these after audio)
  ...
  console.log(`[02-script-generation] ✓ ${wordCount} words, ${parsed.scenes.length} scenes (${storyType}) → ${path.basename(scriptPath)}`);
