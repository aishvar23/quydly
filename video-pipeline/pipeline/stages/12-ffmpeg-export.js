...
async function run(ctx) {
  ...
  if (!remotionVideoPath) {
    throw new Error('[12-ffmpeg-export] No Remotion video path in context');
  }

  console.log('[12-ffmpeg-export] Re-encoding for platform delivery...');
  await encode(remotionVideoPath, exportPath);
  console.log(`[12-ffmpeg-export] ✓ Export → ${path.basename(exportPath)}`);
  return { exportPath };
}
