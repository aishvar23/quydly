...
// Service-role client — DB writes only, never sent to frontend
function buildSupabase() {
  return createClient(process.env.SUPABASE_URL, process.env.SUPABASE_SERVICE_KEY);
}

// Anon-key client — used only to verify the caller's JWT
...
// Streak logic exactly as in SPEC.md
function updateStreak(user, today) {
  ...
  if (user.last_played === today)     return user.streak;     // already played today
...
router.post("/", async (req, res) => {
  ...
  try {
    ...
    // Global rank: count users with more total_points than this user
    ...
    // promptSaveStreak: true tells the frontend to show "Save your streak — sign in with Google"
    // Frontend calls: supabase.auth.linkIdentity({ provider: 'google' })
    // NOTE: requires Google OAuth enabled in Supabase dashboard (Auth → Providers → Google)
